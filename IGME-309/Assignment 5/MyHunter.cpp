...
MyHunter::MyHunter(vec2 _position, int _ID)
{
	...
	bodyColor = vec3(1.0f, 1.0f, 0.0f);
	headColor = vec3(0.0f, 0.0f, 0.1f);
...
void MyHunter::update(float _deltaTime, const vector<Monster*> _monsters, const vector<Hunter*> _players)
{
	if (isActived == true) {
		...
		{
			vec2 direction = normalize(position - nearestMonster->position);
			if (minDis < 80.0f) {

				position = vec2(position.x + direction.x * speed * _deltaTime, position.y + direction.y * speed * _deltaTime);
			}
			else {
				position = vec2(position.x - direction.x * speed * _deltaTime, position.y - direction.y * speed * _deltaTime);
			}

			rotation = degrees(atan(nearestMonster->position.y - position.y, nearestMonster->position.x - position.x));
		}
