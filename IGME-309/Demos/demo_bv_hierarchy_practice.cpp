                                    ...
                                    float x3, float y3, float x4, float y4)
{
    ...
    // TODO: For vertex 1 (x1, y1)
    maxDist = fmaxf(maxDist, sqrt(powf(x1 - centerX, 2) + powf(y1 - centerY, 2)));

    // TODO: For vertex 2 (x2, y2)
    maxDist = fmaxf(maxDist, sqrt(powf(x2 - centerX, 2) + powf(y2 - centerY, 2)));
    // TODO: For vertex 3 (x3, y3)
    maxDist = fmaxf(maxDist, sqrt(powf(x3 - centerX, 2) + powf(y3 - centerY, 2)));
    // TODO: For vertex 4 (x4, y4)
    maxDist = fmaxf(maxDist, sqrt(powf(x4 - centerX, 2) + powf(y4 - centerY, 2)));
...
bool collisionDetection(Circle *node, float cursorX, float cursorY, float cursorRadius, bool &leafHitFlag)
{
    ...
    node->isColliding = distance <= (node->radius + cursorRadius);
    ...
    {
        childHit = collisionDetection(node->children[i], cursorX, cursorY, cursorRadius, leafHitFlag);
    }
